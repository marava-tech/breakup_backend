...
import com.breakupstories.repository.CommentSupportRepository;
...
public class CommentService {
    ...
    public CommentResponse createComment(String userId, CommentRequest request) {
        ...
        if (request.getParentId() != null) {
            ...
            if (parent.getParentId() != null) {
                // Determine if we should error or flatten.
                // "not nested ... only 1 level" implies strict structure.
                throw new RuntimeException("Reply nesting is not allowed. You can only reply to main comments.");
            }
        }
     ...
     */
    @Cacheable(value = "comments", key = "#storyId + ':' + #page + ':' + #size + ':' + #currentUserId")
    public PagedResponse<CommentResponse> getCommentsByStory(String storyId, String currentUserId, int page, int size) {
        log.info("Getting comments for story {} (user: {}, page: {}, size: {})", storyId, currentUserId, page, size);
        ...
        List<CommentResponse> comments = commentPage.getContent().stream()
                .map(comment -> buildCommentWithReplies(comment, currentUserId))
    ...
    public List<CommentResponse> getAllCommentsByStory(String storyId) {
        ...
        return topLevelComments.stream()
                .map(comment -> buildCommentWithReplies(comment, null))
                .collect(Collectors.toList());
    }
     ...
     * @param currentUserId The current user ID for personalizing response (likes)
     * @return CommentResponse with nested replies
     */
    private CommentResponse buildCommentWithReplies(Comment comment, String currentUserId) {
        // Fetch user information to include username
        User user = userService.getUserEntityById(comment.getUserId());

        long supportCount = getSupportCount(comment.getId());
        boolean isSupportedByMe = currentUserId != null && isSupported(currentUserId, comment.getId());

        CommentResponse response = CommentResponse.fromComment(comment, user, supportCount, isSupportedByMe);

        // Only fetch replies if this is a root comment (parentId is null)
        if (comment.getParentId() == null) {
            ...
            // Since we enforce 1-level nesting on create, these replies won't have children
            List<CommentResponse> replyResponses = replies.stream()
                    .map(reply -> buildCommentWithReplies(reply, currentUserId))
                    .collect(Collectors.toList());
    ...
    public long getSupportCount(String commentId) {
        return commentSupportRepository.countByCommentId(commentId);
    }

    public boolean isSupported(String userId, String commentId) {
        return commentSupportRepository.existsByUserIdAndCommentId(userId, commentId);
    }

    @CacheEvict(value = "comments", allEntries = true)
    public void supportComment(String userId, String commentId) {
        log.info("User {} supporting comment {}", userId, commentId);
        if (!commentSupportRepository.existsByUserIdAndCommentId(userId, commentId)) {
            CommentSupport support = CommentSupport.builder()
                    .userId(userId)
                    .commentId(commentId)
                    .build();
            commentSupportRepository.save(support);
        }
    }

    @CacheEvict(value = "comments", allEntries = true)
    public void unsupportComment(String userId, String commentId) {
        log.info("User {} unsupporting comment {}", userId, commentId);
        commentSupportRepository.deleteByUserIdAndCommentId(userId, commentId);
    }
    ...
    public CommentResponse getCommentById(String commentId) {
        ...
        return buildCommentWithReplies(comment, null);
    }
